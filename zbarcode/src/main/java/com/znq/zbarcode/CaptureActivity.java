...
public class CaptureActivity extends Activity implements SurfaceHolder.Callback {
    ...
    public static ArrayList<String> bookIdList=new ArrayList<>();

    private TextView finishScan;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if(getIntent()!=null){
            mResult=getIntent().getStringExtra(EXTRA_STRING);
        }
        initView();
    ...
    private void initView() {
        ...
        finishScan=(TextView)findViewById(R.id.btn_finish_scan);
        if(mResult.equals("readerId")){
            finishScan.setText("取消");
        ...
        }else if(mResult.equals("bookId")){
            finishScan.setText("取消添加");
            finishScan.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    CaptureActivity.this.finish();
...
//        }
        if(mResult.equals("readerId")){
            ...
            intent2.putExtra("readerId", result);
            // 通过调用setResult方法返回结果给前一个activity。
            CaptureActivity.this.setResult(111, intent2);
            //关闭当前activity
            CaptureActivity.this.finish();
        }else if(mResult.equals("bookId")){
//            if(bookIdList.size()<3){
//                bookIdList.add(result);
//                Toast.makeText(this,"添加了一本书籍，还可添加"+(4-bookIdList.size())+"本",Toast.LENGTH_SHORT).show();
//            }else if(bookIdList.size()==3){
//                bookIdList.add(result);
//                Intent intent2 = new Intent();
//                intent2.putExtra("bookIdNum", ""+bookIdList.size());
//                // 通过调用setResult方法返回结果给前一个activity。
//                CaptureActivity.this.setResult(222, intent2);
//                //关闭当前activity
//                CaptureActivity.this.finish();
//            }
            Intent intent2 = new Intent();
            intent2.putExtra("bookId", result);
            // 通过调用setResult方法返回结果给前一个activity。
            CaptureActivity.this.setResult(222, intent2);
            //关闭当前activity
            CaptureActivity.this.finish();
